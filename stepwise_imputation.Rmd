...
# setwd("/Users/dana/Documents/R programming/git")
...
city_country_df
write.csv(city_country_df , "city_country_df.csv", row.names = FALSE)
...
write.csv(best_citizens_df, "best_citizens_df.csv", row.names = FALSE)
...
collect_individuals <- function(cities_filled_df){
  data_filled_df <- (
    ...
    %>% mutate(top_citizenship=as.character(coalesce(citizenship_2, citizenship_1)),
               ...
               imp_birth_country=case_when(!is.na(imp_birth_country) ~ imp_birth_country, TRUE ~ top_citizenship),
...
print(data_filled_df
      ...
      %>% mutate(
        citizenship_1 = case_when(citizenship_1=="0"~"000", TRUE ~ as.character(citizenship_1)),
...
german_names_in_Poland_0<- read.csv("Deutscher name.csv", stringsAsFactors = FALSE)
```
...
german_names_in_Poland_0<- german_names_in_Poland_0 %>%
  mutate(
    Polish_name_clean = Polisch.name %>%
      str_remove(",.*$") %>%  
      str_remove_all("[´]") %>%  
      str_remove_all("\\[[a-zA-Z]\\]") %>%
      stri_trans_general("Latin-ASCII") %>%          #  (ł → l, ś → s и т.п.)
...
german_names_in_Poland_0<- german_names_in_Poland_0 %>%
  mutate(
    German_name_clean = Deutscher.name %>%
      str_remove(",.*$") %>%  
      str_remove_all("[´]") %>% 
      str_remove_all("\\[[a-zA-Z]\\]") %>%
      str_remove_all("\\s*\\([^)]*\\)") %>%         # remove "( ... )"
      str_remove("\\s*->.*$") %>%                   # 🔹 remove "-> SIEHE ..."
      stri_trans_general("Latin-ASCII") %>%          
      toupper() %>%                                  
      str_replace_all("(JETZT|\\sJE[TZ]+T|\\sJ\\.|\\sJ\\s)+", "/") %>% 
      str_replace_all("\\s+(KRS|KREIS|KRS\\.|K[\\.\\s])\\s+", "/") %>% 
      str_remove("\\s+(0T|ORTSTEIL|STADT|BEZIRK|GEMEINDE).*$") %>%
      str_trim()                                    # 🔹 clean trailing spaces
  )
...
```{r}
pol_lookup_1 <- setNames(
 german_names_in_Poland_0$`Polish country`,
  german_names_in_Poland_0$German_name_clean
)

multiple_cities_poland_1 <- multiple_cities_poland_0 %>%
  mutate(
    city_1_country_1 = pol_lookup_1[city_1],
    city_2_country_1 = pol_lookup_1[city_2],
    city_3_country_1 = pol_lookup_1[city_3],
    city_4_country_1 = pol_lookup_1[city_4]
  ) %>%
  rowwise() %>%
  mutate(
    exonym_country_1 = resolve_exonym_country(
      city_1_country_1, city_2_country_1, city_3_country_1, city_4_country_1
    )
  ) %>%
  ungroup()

multiple_cities_poland_1_small <- multiple_cities_poland_1 %>%
  select(pid, exonym_country_1)
...
```{r}
multiple_cities_poland<- multiple_cities_poland %>%
  left_join(multiple_cities_poland_small, by = "pid") %>%
  left_join(multiple_cities_poland_0_small, by = "pid") %>%
  left_join(multiple_cities_poland_1_small, by = "pid")
...
multiple_cities_poland <-multiple_cities_poland %>%
  rowwise() %>%
  mutate(
    history_birth_country = resolve_exonym_country(
    exonym_country_0, exonym_country_1, exonym_country.x,exonym_country.y)
  ) %>%
  ungroup()

multiple_cities_poland_small <- multiple_cities_poland %>%
  select(pid,   history_birth_country )

write_csv(multiple_cities_poland, "multiple_cities_poland.csv")
...
multiple_cities<-multiple_cities %>%
  left_join(multiple_cities_poland_small, by = "pid")%>%
  filter(is.na(history_birth_country))
...
data_df <- data_df %>%
  left_join(
    multiple_cities_poland_small %>% select(pid, history_birth_country),
    by = "pid"
  )
```

```{r}
match_cities_new <- function(data_df) {
  
  cities_filled_df <- data_df %>%
    
    # Initial imputation from birth_country and history_birth_country
    mutate(
      imp_birth_country = coalesce(birth_country, history_birth_country),
      imp_type = case_when(
        !is.na(birth_country) ~ "given",
        is.na(birth_country) & !is.na(history_birth_country) ~ "history",
        TRUE ~ NA_character_
      )
    ) %>%
    ...
    # German list matches
    left_join(
      deu_cities_df %>% rename(birth_city = deu_city, imp_deu = imp_type),
      by = "birth_city"
    ) %>%
    ...
    left_join(
      states_df %>%
        select(lit_state = bev_state, name) %>%
        mutate(country_literal = TRUE) %>%
        mutate(name = name %>% stri_trans_general("Latin-ASCII") %>% toupper()),
      by = c("birth_city" = "name")
    ) %>%
    ...
    left_join(
      states_df %>% 
        select(world_id = bev_state, iso_3) %>% distinct() %>%
        filter(!(world_id %in% c(133, 276))),
      join_by(iso3 == iso_3)
    ) %>%
    
    # Internally assigned city-countries
    left_join(
      best_countries_df %>% rename(city_birth_country = birth_country),
      by = "birth_city"
    ) %>%
    
    # Expanded German processing
    mutate(base_city = clean_german_city(birth_city)) %>%
    left_join(
      deu_base_cities_df %>% rename(base_city = deu_city, imp_deu_expanded = imp_type),
      by = "base_city"
    ) %>%
    
    # Citizenship-based cities
    left_join(best_citizens_df %>% filter(total_city_records > 1), by = "birth_city") %>%
    
    # Core imputation rules
    mutate(
      imp_birth_country = case_when(
        !is.na(imp_birth_country) ~ imp_birth_country,
        !is.na(imp_deu) ~ "000",
        !is.na(bev_state) ~ bev_state,
        coalesce(country_literal, FALSE) ~ lit_state,
        !is.na(ostgebiete) ~ ostgebiete,
        !is.na(world_id) ~ world_id,
        !is.na(imp_deu_expanded) ~ "000",
        pct_country >= 50 ~ city_birth_country,
        pct_citizen >= 70 ~ citizenship,
        !is.na(history_birth_country) ~ history_birth_country,
        TRUE ~ NA_character_
      ),
      imp_type = case_when(
        !is.na(imp_type) ~ imp_type,
        ...
        !is.na(imp_deu) ~ imp_deu,
        !is.na(imp_deu_expanded) ~ "german_base",
        !is.na(bev_state) ~ "country_literal", 
        coalesce(country_literal, FALSE) ~ "country_literal",
        !is.na(ostgebiete) ~ "ostgebiete",
        pct_country == 100 ~ "unique_city",
        pct_country >= 50 ~ "city_o50",
        !is.na(world_id) ~ "world_list",
        pct_citizen == 100 ~ "unique_citizen",
        pct_citizen >= 70 ~ "citizen_o70",
        !is.na(history_birth_country) ~ "history",
        TRUE ~ imp_type
      )
    ) %>%
...
```{r}
# Convert back to individuals, marked with matched country(s) and fill with citizenship
collect_individuals_new <- function(cities_filled_df){
  data_filled_df <- (
    cities_filled_df
...
cities_filled_df <- match_cities_new(data_df)
data_filled_df <- collect_individuals_new(cities_filled_df)
write.csv(data_filled_df, "data_filled_df.csv", row.names = FALSE)
```
