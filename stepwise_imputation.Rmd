...
multiple_cities <- multiple_cities %>%
  ...
  mutate(
    history_birth_country = resolve_exonym_country(
      exonym_country.x, exonym_country.y, exonym_country.x.x, exonym_country.y.y,exonym_country_0,
    exonym_country_1)
  ) %>%
  ungroup()
...
```{r}
data_filled_df <- data_filled_df%>%
  left_join(
    multiple_cities %>% select(pid, history_birth_country),
    by = "pid"
  )
```
...
data_filled_df <- data_filled_df %>%
  mutate(
    imp_birth_country = case_when(
      imp_birth_country == "000/152" & !is.na(history_birth_country) ~ "152",
      TRUE ~ imp_birth_country
    ),
    imp_name = case_when(
      imp_birth_country == "000/152" & !is.na(history_birth_country) ~ "Polen",
      TRUE ~ imp_name
    ),
    imp_type = case_when(
      imp_birth_country == "000/152" & !is.na(history_birth_country) ~ paste0(imp_type, "/history"),
      TRUE ~ imp_type
    )
  )
```
