...
ostgebiete_df <-rbind(data.frame(
  ...
  # Kaliningrad
  birth_city = c("GERDAUEN", "SAMLAND","KONIGSBERG"),
  ostgebiete = "160"
)
)
ostgebiete_df
...
```{r}
# Get the first non-missing birth city
# Select rows with multiple countries and extract the first city
first_cities <- data_filled_df %>%
  filter(str_detect(imp_birth_country, "/")) %>%
  
  # Split birth_city on "/" into up to 4 parts
  separate(birth_city,
...
```{r}
# Join first_cities with world_cities_df to get the country
first_cities_with_country <- first_cities %>%
  left_join(world_cities_df, by = c("city_1" = "name"))

first_cities_with_country%>%
  filter(imp_birth_country=="000/160")
...
#OSTPR. means Ostpreußen
```
#Determ Russia
````{r}
first_cities_with_country <- first_cities_with_country %>%
  mutate(
    Russian_country = case_when(
      city_1 %in% c("KONIGSBERG", "SAMLAND", "GERDAUEN") ~ "Russia",
      city_2 %in% c("KONIGSBERG", "SAMLAND", "GERDAUEN") ~ "Russia",
      TRUE ~ NA_character_
    )
  )

# first_cities_with_country<-first_cities_with_country%>%
#   filter(is.na(first_cities_with_country$Russian_country))
````
...
first_cities_with_country <- first_cities_with_country %>%
  mutate(
    Poland_country_0 = case_when(
      city_1 %in% c("GROTTKAU", "SCHRODA", "SCHLESIEN", "OSTPR.", "OSTPREUSSEN", "NEIDENBURG","LABIAU","RASTENBURG")~ "Poland",
      city_2 %in% c("GROTTKAU", "SCHRODA", "SCHLESIEN", "OSTPR.", "OSTPREUSSEN", "NEIDENBURG","LABIAU","RASTENBURG") ~ "Poland",
      TRUE ~ NA_character_
    )
  )

# first_cities_with_country<-first_cities_with_country%>%
#   filter(is.na(first_cities_with_country$Poland_country_0))
````
...
poland <- left_join(
  ...
  german_names_in_Poland,
  by = c("city_1" = "German_name_clean")
)

poland <- poland %>%
  rename(Poland_country = Country)
poland%>%
  filter(!is.na(Poland_country))
...
ostpruss_df <- ostpruss_df %>%
  ...
  mutate(`Prussian country` = "Poland")



poland_prus<-left_join(poland,ostpruss_df,by=c("city_1"="German_name_cleaned"))

poland_prus%>%
  filter(is.na(poland_prus$`Prussian country`))%>%
  filter(is.na(Polish_name))%>%
  group_by(city_2)%>%
  summarise(n = n(), .groups = "drop") %>%
  filter(n > 1) %>%
  arrange(desc(n))
...
poland_prus_left<-left_join(poland_prus%>%
  filter(is.na(poland_prus$`Prussian country`))%>%
  filter(is.na(Polish_name))
,
ostpruss_df, by=c("city_2"="German_name_cleaned"))

left<-poland_prus_left%>%
  filter(is.na(`Prussian country.y`))
...
````{r}
# Normalize exonym reference
exonym_ref <- german_names_in_Poland %>%
  select(German_name_clean, Polish_name_clean, Country) %>%
  bind_rows(
    ostpruss_df %>%
      select(German_name_clean = German_name_cleaned,
             Polish_name_clean = Polish_name_cleaned,
             Country = `Prussian country`)
  ) %>%
  filter(!is.na(German_name_clean)) %>%
  distinct(German_name_clean, .keep_all = TRUE) %>%
  mutate(
    Country = case_when(
      Country %in% c("Poland", "POLAND") ~ "152",   # UN M49 / ISO code
      Country %in% c("Russia", "RUSSIA") ~ "160",
      TRUE ~ Country
    )
  )

exonym_ref <- exonym_ref %>%
  rename(Exonym_country = Country)

names(exonym_ref)

write.csv(exonym_ref, "exonym_ref.csv", row.names = FALSE)


# # Impute into main dataset
# first_cities_with_country <- first_cities_with_country %>%
#   left_join(exonym_ref %>% select(German_name_clean, Country),
#             by = c("city_1" = "German_name_clean")) %>%
#   rename(country_from_city1 = Country) %>%
#   left_join(exonym_ref %>% select(German_name_clean, Country),
#             by = c("city_2" = "German_name_clean")) %>%
#   rename(country_from_city2 = Country) %>%
#   mutate(
#     imp_birth_country = case_when(
#       !is.na(country_from_city1) ~ country_from_city1,
#       !is.na(country_from_city2) ~ country_from_city2,
#       TRUE ~ imp_birth_country   # keep existing if nothing matched
#     )
#   )
...
# ````{r}
# # Imputation step in your main dataset
# first_cities_with_country <- first_cities_with_country %>%
#   left_join(exonym_ref %>% select(German_name_clean, Country),
#             by = c("city_1" = "German_name_clean")) %>%
#   rename(country_from_city1 = Country) %>%
...
#       !is.na(country_from_city1) ~ country_from_city1,
#       !is.na(country_from_city2) ~ country_from_city2,
#       TRUE ~ imp_birth_country   # fallback to existing
#     )
#   )
# ````
