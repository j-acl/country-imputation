...
- report
  - data, bib
  - major imputation steps, refactor to funnel format?
...
- improve matching
  ...
  - re-examine slash splitting -- keep only district? or handle in the collect...
  - transitive property
...
best_countries_df
print(multi_country_df %>% filter(birth_city=="IRAN"))
...
cities_filled_df <- (
  ...
  # Ostgebeit matches
  # join on ostgebeit lookup
  # (reconsider district parsing)
  ...
  %>% left_join(
    states_df %>% select(bev_state, name) %>% rename(lit_state=bev_state) %>% mutate(country_literal=TRUE)
    %>% mutate(lit_state = name %>% stri_trans_general("Latin-ASCII") %>% toupper()),
...
print(
  ...
  %>% filter(str_detect(birth_city.y,"RASTEN"))
)
